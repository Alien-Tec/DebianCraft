...
apply_dunst() {
	...
	sed -i ${PATH_DUNST}/dunstrc \
		-e "s/geometry = .*/geometry = $dunst_geometry/g" \
...
apply_plank() {
	...
	cat > "$HOME"/.cache/plank.conf <<- _EOF_
		...
		dock-items=['com.alacritty.Alacritty.dockitem', 'firefox.dockitem', 'thunar.dockitem', 'geany.dockitem', 'xfce4-terminal.dockitem', 'xfce-settings-manager.dockitem']
		hide-delay=0
...
# Notify User -------------------------------
notify_user() {
	dunstify -u normal -h string:x-dunst-stack-tag:applytheme -i /usr/share/debiancraft/icons/dunst/themes.png "Applying Style : $THEME"
}
